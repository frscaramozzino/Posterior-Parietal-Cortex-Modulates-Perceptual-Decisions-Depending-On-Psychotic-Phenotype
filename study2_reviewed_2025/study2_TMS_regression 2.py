...
data['z_pdi']=(data.pdi - data.pdi.mean())/data.pdi.std(ddof=0)
data['z_caps']=(data.caps - data.caps.mean())/data.caps.std(ddof=0)
...
data['z_pdi']=(data['pdi'] - data['pdi'].mean()) / data['pdi'].std()


data['z_caps']=(data['caps'] - data['caps'].mean()) / data['caps'].std()
...
#%%
#cond
mreg_avt_cond= hddm.models.HDDMRegressor(data,
...
#%%
#cond,TMS
mreg_avt_tms_cond= hddm.models.HDDMRegressor(data,
...
mreg_avt_tms_cond2= hddm.models.HDDMRegressor(data, ['a ~ 1+coherence+session+coherence*session', 
...
#cond,PDI
mreg_avt_coh_tms_PDI= hddm.models.HDDMRegressor(data, ['a ~ 1+coherence+z_pdi+session+z_pdi*session', 
...
mreg_avt_coh_tms_PDI.plot_posteriors()
...
mreg_avt_tms_PDI= hddm.models.HDDMRegressor(data, ['a ~ 1+z_pdi+session+z_pdi*session', 
...
#################
#cond,CAPS
mreg_avt_tms_CAPS= hddm.models.HDDMRegressor(data, ['a ~ 1+z_caps+session+z_caps*session', 
